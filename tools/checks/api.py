...
def check_browser_scope(src_root):
    ...
    script = content_script(7, '30d WN8', False)
    ...
    check('the content script carries its generation and settings',
          decoded.startswith('(function(GENERATION, RATING_TITLE, SHOW_RATING){')
          and decoded.endswith('})(7, "30d WN8", false);'))
    ...
    pushed = answers_script({
        'RASZ': {'flags': ['data:image/png;base64,iVBOR+/w=='],
                 'score': {'value': 1792.2, 'color': '#6D9521'}},
        'TENTS': {'flags': [], 'score': None}})
